## Assessment in Data Camp of for Association and Chi-Squared Test.
...
# 1. Filter 'errors' for grades 'A-' and 'C-'.

# 2. Group the data by grade and hit.

# 3. Summarize the number of hits for each grade.

# 4. Generate a two-by-two table containing the number of hits and misses for each grade. Try using the 'spread' function to generate this table.

# 5. Calculate the proportion of times each grade was correct.
...
# 1. Use the 'chisq.test' function to perform the chi-squared test. Save the results to an object called 'chisq_test'.

# 2. Print the p-value of the test to the console.
...
# 1. Calculate the odds that a grade 'C-' poll predicts the correct winner. Save this result to a variable called 'odds_C'.

# 2. Calculate the odds that a grade 'A-' poll predicts the correct winner. Save this result to a variable called 'odds_A'.

# 3. Calculate the odds ratio that tells us how many times larger the odds of a grade 'A-' poll is at predicting the winner than a grade 'C-' poll.
...
# c) The p-value is below 0.05, but the odds ratio is very close to 1. There is not a scientifically significant difference in performance. (correcto)
